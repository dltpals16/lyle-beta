...
DECIDE_PRODUCTS_PROMPT = """사용자 발화와 상황에 맞는 추천 제품 종류를 결정하세요.
...
## 작업
이 사용자에게 가장 적합한 제품 종류 **정확히 3개**를 선택하세요 (우선순위 순).

⚠️ **사용자 발화에 구체 제품 타입(음료/차/체온계/마사지볼 등)이 있으면 반드시 그 타입을 반영**할 것.
   - "음료 추천" → 영양 보충 성분이 아니라 실제 음료(차/즙/티백 등)로
   - "차 추천" → 차 종류로
   - "체온계 추천" → 체온계 종류로 (영양제 아님)

## 카테고리별 예시
- **영양제**: 성분명 (예: "활성형 엽산", "비타민 D3", "마그네슘 비스글리시네이트")
- **건강식품 (일반)**: 콜라겐, 단백질 분말, 즙류, 환 등
- **건강식품 (음료/차)**: "임산부 보리차", "캐모마일 티", "비트즙", "콜라겐 음료" 등 음료 형태
...
2. **성분/기능 중심 설명** — 추천 종류(예: 활성형 엽산, 비타민 D3) 각각:
   - 어떤 효과/장점이 있는지
   - 왜 이 사용자에게 맞는지 (사용자 상황과 연결)
3. **구체적 상품명/브랜드명은 본문에 적지 말 것** — 카드로 따로 표시되니 본문에서 콕 집어 광고처럼 추천하면 안 됨
   - ❌ "'락티브 활성형 엽산 800μg DFE' 같은 제품을 추천드려요"
   - ✅ "대표적인 활성형 엽산 제품들은 아래 카드에서 확인하실 수 있어요"
4. URL/가격도 본문에 적지 말 것
5. 마지막: 안전 안내 (한 줄)

## 답변 톤 예시
"체외수정 준비 중이시니 배아 질 향상에 도움될 영양제 몇 가지 알려드릴게요.

**활성형 엽산** — 세포 분열과 DNA 합성에 핵심 역할. 일반 엽산보다 흡수율이 높아 임신 준비기에 권장돼요.
**비타민 D3** — 호르몬 균형과 면역에 영향. 체외수정 성공률 관련 연구도 있어요.
**코엔자임 Q10** — 항산화 작용으로 난자/세포 질 보호에 도움.

추천드릴 만한 대표 제품들은 아래 카드에서 확인해 보세요.
복약 중이거나 임신/수유 중이시면 의사·약사 상담 후 복용하시는 게 안전해요."
...
class RecommendAgent:
    ...
    def _decide_products(self, state: RecommendAgentState) -> dict:
        ...
        if not isinstance(products, list):
            products = []
        # 정확히 3개
        products = [p for p in products if isinstance(p, str)][:3]
    ...
    def _search_shopping(self, state: RecommendAgentState) -> dict:
        ...
        target_products(정확히 3개)마다 1개씩 → 최종 카드 3개.
        검색 0건인 제품은 다른 제품의 2위로 보충.
        """
        seen_pids: set = set()
        picked: list[Product] = []
        fallbacks: list[Product] = []   # 보충용 (2위 이하)

        for query in state["target_products"]:
            try:
                ps = self.shopping_search.search(query=query, max_results=5)
            except Exception as e:
            ...
            chosen = False
            for p in ps:
                if p.product_id and p.product_id in seen_pids:
                    continue
                if not chosen:
                    seen_pids.add(p.product_id or p.link)
                    picked.append(p)
                    chosen = True
                else:
                    fallbacks.append(p)

        # 일부 제품이 0건이면 fallback으로 보충해서 3개 채움
        for p in fallbacks:
            if len(picked) >= 3:
                break
            pid = p.product_id or p.link
            if pid in seen_pids:
                continue
            seen_pids.add(pid)
            picked.append(p)
        ...
        picked = picked[:3]
        print(f"[RecommendAgent] 쇼핑 결과: {len(picked)}건 (target {len(state['target_products'])}개)")
        return {"shopping_results": [_product_to_dict(p) for p in picked]}
